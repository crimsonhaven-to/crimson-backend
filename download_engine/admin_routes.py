...
class DownloadCreate(BaseModel):
    url: str = Field(
        ..., min_length=1, max_length=8000, description="An http(s) URL or a magnet: link"
    )
...
def _kind(url: str) -> str:
    ...
    if low.startswith(("http://", "https://")):
        if low.split("?", 1)[0].endswith(".torrent"):
            raise HTTPException(
                status_code=400, detail="Paste the magnet link instead of a .torrent file URL."
            )
        return KIND_HTTP
        ...
        roots = [
            {
                ...
                "label": r.get("label"),
                "path": r["path"],
                **inspect_downloads(r["path"]),
            }
            for r in local_store.download_roots_config()
        ]
        return store.stats(), roots

    (stats, targets), aria2_ok = await asyncio.gather(
        asyncio.to_thread(_read), aria2.is_available()
    )
...
async def list_download_jobs(
    status: Optional[str] = Query(
        None, description="pending / active / paused / complete / failed"
    ),
...
async def create_download(
    request: Request, body: DownloadCreate, user: dict = Depends(require_admin)
):
