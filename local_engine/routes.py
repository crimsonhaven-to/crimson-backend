...
async def get_local_library(request: Request):
    if not is_configured():
        return json_response.gzip_json(request, {
...
async def search_local(query_name: str = Query(..., min_length=1, description="Local title/filename to search")):
    ...
    items = await asyncio.to_thread(_cached_items)
    matches = search_library(query_name, items=[enrichment.apply_cached(it) for it in items], limit=20)
    suggestions = [
        {
            "id": m["id"],
            "title": m["title"],
...
async def watch_local(request: Request, token: str, title: Optional[str] = Query(None)):
    ...
    async def _lines():
        yield ndjson.line(build_meta_line(
            tmdb_id=0, season_number=None, episode_number=None, anilist_id=None, title=title,
        ))
        try:
            ...
            rel = None
        if rel:
            yield ndjson.line(build_stream_line({
                "source": "Local",
