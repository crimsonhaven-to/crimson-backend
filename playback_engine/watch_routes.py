...
@limiter.limit("30/minute")
async def deprecated_watch(
    request: Request, anilist_id: int, episode_number: int, season_part: int = Query(1)
):
    ...
    if season_number is None:
        ...
        if movie_id:
            ...
            return _stream(
                watch_events(
                    movie_id, None, None, None, title, base_url=base_url, media_type="movie"
                )
            )
    ...
    return _stream(
        watch_events(
            tmdb_id,
            season_number if season_number is not None else 0,
            episode_number,
            anilist_id,
            base_url=base_url,
        )
