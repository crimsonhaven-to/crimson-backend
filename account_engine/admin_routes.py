...
def _public_user(row: Optional[dict]) -> Optional[dict]:
    ...
    if not row:
        return None
    out = {k: v for k, v in row.items() if k not in ("password_hash", "public_key", "session_expires_at")}
    return {
        **out,
...
async def security_stats(days: int = Query(14, ge=1, le=90, description="Window for the chart/aggregates")):
    """24-hour tiles, a daily series, per-type totals, the top offending IPs and
    the most-targeted identities."""
    return {"success": True, "generated_at": utc_now_iso(), **await asyncio.to_thread(audit.stats, days)}
...
async def security_events(
    ...
    data = await asyncio.to_thread(audit.list_events, event_type, outcome, ip, search, hours, limit, offset)
    return {
        "success": True,
        "generated_at": utc_now_iso(),
...
def _apply_user_update(request: Request, admin: dict, user_id: int, target: dict, body: UserUpdate) -> None:
    ...
    if body.is_admin is not None and bool(target.get("is_admin")) != body.is_admin:
        ...
        log_admin_action(request, admin, "admin_granted" if body.is_admin else "admin_revoked", **audit_target)

    if body.email_verified is not None:
        store.set_email_verified(user_id, body.email_verified)
        log_admin_action(request, admin, "verified_set" if body.email_verified else "verified_cleared", **audit_target)
    ...
    if body.chat_enabled is not None and bool(target.get("chat_enabled")) != body.chat_enabled:
        ...
        log_admin_action(request, admin, "chat_granted" if body.chat_enabled else "chat_revoked", **audit_target)
    ...
    if body.chat_budget_reset:
        ...
        log_admin_action(request, admin, "chat_budget_set", budget=body.chat_monthly_token_budget, **audit_target)
...
async def update_user(request: Request, user_id: int, body: UserUpdate, user: dict = Depends(require_admin)):
    ...
    return {"success": True, "user": _public_user(await asyncio.to_thread(store.get_account, user_id))}
...
async def revoke_user_sessions(request: Request, user_id: int, user: dict = Depends(require_admin)):
    ...
    log_admin_action(request, user, "sessions_revoked", target_user_id=user_id, target=target.get("email"))
...
async def delete_user(request: Request, user_id: int, user: dict = Depends(require_admin)):
    ...
    if not await asyncio.to_thread(store.delete_account, user_id):
        raise HTTPException(status_code=404, detail="User not found")
    log_admin_action(request, user, "user_deleted", target_user_id=user_id, target=(target or {}).get("email"))
...
async def _run_broadcast(recipients: list, subject: str, message: str) -> None:
    async with _broadcast_lock:
        ...
        try:
            result = await asyncio.to_thread(mailer.send_broadcast, recipients, subject, message, _progress)
...
async def send_broadcast_email(request: Request, body: BroadcastEmail, user: dict = Depends(require_admin)):
    ...
    log_admin_action(request, user, "broadcast_email", subject=body.subject,
    ...
    _broadcast_state.update(
        running=True, started_at=utc_now_iso(), finished_at=None, subject=subject,
        total=len(recipients), sent=0, failed=0, triggered_by=admin_identity(user),
    )
...
async def create_invites(request: Request, body: InviteCreate, user: dict = Depends(require_admin)):
    ...
    codes = await asyncio.to_thread(store.create_invite_tokens, body.count, admin_identity(user), ttl)
