"""
The site-wide login wall: what it lets through and what it stops.

``api.LoginWallMiddleware`` is the only thing standing between the internet and
roughly 140 routes, and its whitelist is three hand-maintained collections
(``_PUBLIC_EXACT``, ``_PUBLIC_PREFIXES``, ``_DYNAMIC_PUBLIC_PREFIXES``). Nothing
verified any of it until this module: a path added to the wrong collection, or a
middleware reordering, was a production-only discovery.

Most tests here drive the middleware over a **stub** ASGI app rather than the
real one, for two reasons. It keeps the suite's no-network/no-database property
(the real ``/health`` queries Postgres), and it makes the assertion exact: a
200 means *the wall passed the request through*, where against the real app a
public path may legitimately answer 404 or 503 and "not 401" is the most that
could be claimed. A handful of tests at the bottom do use the real app, to prove
the middleware is actually mounted and ordered correctly.

Lifespan is never run. ``httpx.ASGITransport`` does not start it; wrapping the
app in ``TestClient`` as a context manager would, and that would reach for the
database.
"""
...
# base build. Hard-coding () would fail an operator build for a reason that has
# nothing to do with the wall.
PUBLIC_EXACT = sorted(api._PUBLIC_EXACT)
PUBLIC_PREFIXES = tuple(api._PUBLIC_PREFIXES) + tuple(api._DYNAMIC_PUBLIC_PREFIXES)
...
def _client(app=None):
    ...
    wall = api.LoginWallMiddleware(app or _stub_app)
...
def _no_valid_credentials(monkeypatch):
    ...
    async def _reject(_value):
        return False
    monkeypatch.setattr(api, "_session_is_valid", _reject)
    monkeypatch.setattr(api, "_apikey_is_valid", _reject)
...
async def test_no_gated_path_is_accidentally_whitelisted():
    ...
    for path in GATED_PATHS:
        assert path not in api._PUBLIC_EXACT
...
async def test_a_valid_session_unlocks_a_gated_path(monkeypatch):
    async def _accept(_token):
        return True
    monkeypatch.setattr(api, "_session_is_valid", _accept)
...
async def test_api_key_unlocks_the_mw_bridge(monkeypatch):
    ...
    async def _accept_keys(_key):
        return True
    monkeypatch.setattr(api, "_session_is_valid", _reject_sessions)
    monkeypatch.setattr(api, "_apikey_is_valid", _accept_keys)
...
async def test_prefix_matching_is_not_segment_aware(monkeypatch):
    """``path.startswith(_PUBLIC_PREFIXES)`` is a string prefix test, not a path
