...
# --- days, streaks and the viewer's timezone ---------------------------------
...
def test_an_absurd_offset_is_clamped(build):
    stats = build(FakeConn(since=None), offset_minutes=99999)
    assert stats["offset_minutes"] == wrapped.MAX_OFFSET_MINUTES
...
# --- parsing the account tables' ISO-TEXT timestamps -------------------------
