...
database (or ffmpeg/TMDB) is needed.
"""
...
def _point_roots_at(monkeypatch, root, *, label="Media", encoding=False):
    """One enabled root, no DB. library and fs share the store singleton, so this
    one patch covers scanning, playability and labels."""
    monkeypatch.setattr(
