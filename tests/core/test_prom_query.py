"""Tests for core/prom_query.py, with the HTTP call stubbed.

What rots here: a typo in the panel catalogue (an unexpanded placeholder, a
``sum()`` over a cluster-wide metric) that still draws a plausible chart; NaN
samples, which are the normal case for ratio panels and would emit invalid JSON;
and the trust boundary, where the browser sends a key and never an expression.
"""
...
def prom(monkeypatch):
    ...
    def _install(payload, status=200):
        ...
        monkeypatch.setattr(prom_query, "http_client", _shared)
        return client
