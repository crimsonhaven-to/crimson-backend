...
Keying on ``themoviedb_id.tv`` alone loses most of a franchise's side content.
Overlord is the worked example throughout:

  * "The Sacred Kingdom" carries ``{"movie": [1014505]}`` and no ``tv`` key, so it
    has no show to group under;
  * "The Dark Hero" and the Ple Ple Pleiades shorts carry no external id at all,
    so Fribb offers nothing to group them by.
...
from metadata_engine.fribb import group_by_show, relation_extras, tmdb_movie_id, tmdb_tv_id
...
def test_movie_id_read_from_the_list_form():
    ...
    assert tmdb_movie_id({"themoviedb_id": {"movie": [1014505]}}) == 1014505


def test_movie_id_is_none_for_a_show():
    assert tmdb_movie_id({"themoviedb_id": {"tv": TMDB_SHOW}}) is None
    assert tmdb_movie_id({"themoviedb_id": None}) is None
    assert tmdb_movie_id({"themoviedb_id": {"movie": []}}) is None
...
def test_a_film_is_never_mistaken_for_a_show():
    ...
    assert tmdb_tv_id({"themoviedb_id": {"movie": [1014505]}}) is None


def test_film_attaches_to_its_show_through_tvdb_id():
    groups, movie_ids, orphans = group_by_show(OVERLORD)
...
def test_a_tvdb_attached_film_never_claims_a_season_slot():
    ...
    groups, _movie_ids, _orphans = group_by_show(OVERLORD)
...
def test_a_film_with_no_parent_becomes_a_standalone_entry():
    ...
    groups, movie_ids, orphans = group_by_show(standalone)
...
def test_an_entry_with_no_ids_at_all_is_left_to_the_relations_pass():
    ...
    groups, movie_ids, orphans = group_by_show(
...
def test_an_unknown_tvdb_id_does_not_invent_a_show():
    groups, _movie_ids, orphans = group_by_show(
...
def test_relations_recover_side_content_fribb_cannot_key():
    ...
    found = relation_extras(season_rows, [], metadata)
...
def test_crossovers_and_sequels_are_not_side_content():
    ...
    assert relation_extras(season_rows, [], metadata) == []
...
def test_an_extra_already_known_from_fribb_is_not_added_twice():
    ...
    assert relation_extras(season_rows, extra_rows, metadata) == []
...
def test_the_same_extra_reached_from_two_seasons_is_added_once():
    ...
    assert [a for _t, a, _f, _n in relation_extras(season_rows, [], metadata)] == [21305]
...
def test_missing_or_malformed_relations_are_survivable():
    ...
    assert relation_extras(season_rows, extra_rows=[], al_metadata=metadata) == []
